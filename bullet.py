...
class BulletEnemy(pygame.sprite.Sprite):
    def __init__(self, pos, direction):
        ...
        # загрузка спрайтов
        self.import_bullet_assets()
        self.image = self.animations['fireball'][0]
    ...
    def import_bullet_assets(self):
        # создание словаря со списками нарезанных по кадрово спрайтами
        character_path = "graphics/enemies/"
        self.animations = {'fireball': [], 'explosion': []}
    ...
    def animate(self):
        ...
        # если пуля проиграла анимация попадания, то она уничтожается
        if self.frame_index >= len(animation) and self.status == 'explosion':
            self.kill()
        # циклическая анимация если пуля летит
        if self.frame_index >= len(animation):
            self.frame_index = 0
